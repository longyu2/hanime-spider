...
def html_to_json(htmltext):
    ...
    for div in divs:
        # print(div.contents)
        content_img_url = div.find('img').get('data-srcset')   # 获取每个本子封面的url，这个url中的数字就是存放源图片的地址
        img_url_number = ''.join(filter(str.isdigit,content_img_url))
        content_article_url = div.find('a').get('href')  # 文章网址
        content_article_name = div.find('div',class_='comic-rows-videos-title').string

        arrs_page.append({'img_url_number':img_url_number,'content_article_url':content_article_url,'content_article_name':content_article_name})

    jsonstr = json.dumps(arrs_page,ensure_ascii=False)  # 将一页的所有数据存入json文件,这代表搜索某个关键词后，整页的文章的信息，还需要根据页数才能拥有所有索引


    return jsonstr
    # jsonname = input("请输入json文件名")
    # with open(jsonname+'.json','w',encoding='utf8') as fp:
    #     fp.write(jsonstr)


# text = ""
#
# html_to_json(get_html())


# 前面只get到了一页的信息，这次获取一个关键词的所有页
# 思路是根据tag上的数量判断有几页,首先获取首页的html,再从首页中提取出带有数量的那个标签
def getALL():
    inp = input("请输入标签名：目前只支持根据标签下载，请输入准确的标签！")

    home_url = "https://hanime1.me/characters/"+inp.replace(" ",'%20')     # 空格转化成url中%20
    ...
    tag_count = tag_count.find(attrs={'class':'no-select'})

    #  将tag名和count保存到字典
    tag_counts = []
    for i in tag_count:
        tag_counts.append(i.string)
    tag_and_count = {'tag_name':tag_counts[0],'count':tag_counts[1]}
    print(tag_and_count)
