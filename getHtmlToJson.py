...
def get_html(httpurl):
    r = requests.get(httpurl, proxies=get_proxy.requests_proxy, headers=headers.headers, cookies=headers.cookies,verify=False)
    return r.text
    
# 本函数用来解析html，并获取其中的信息
def html_to_information(htmltext):
...
def by_tag_get_aLL(tag):
    ...
    for i in range(1, page_count+1):
        ...
        json_page = json.loads(html_to_information(get_html(page_url)))
    ...
    return ALL_PAGE_JSON




# 将已转化好的数据存储为json
def save_json(tag,output_folder):
    ALL_PAGE_JSON =  by_tag_get_aLL(tag)
    with open(output_folder+'/tag['+tag+'].json', 'w', encoding='utf8') as fp:
